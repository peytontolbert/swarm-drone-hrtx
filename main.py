...
def main():
    simulation = CustomDroneEnv(
        num_drones=DEFAULT_NUM_DRONES, gui=DEFAULT_GUI
    )  # Add missing parameters
    while not simulation.is_completed():  # Main simulation loop
        observations = (
            simulation.get_observations()
        )  # Get current state observations from the simulation
        actions = simulation.mimo_transformer.transform_command(
            observations
        )  # Directly use MIMO transformer to generate actions based on observations
