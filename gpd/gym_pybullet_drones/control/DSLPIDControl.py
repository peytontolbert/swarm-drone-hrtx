...
from gpd.gym_pybullet_drones.control.BaseControl import BaseControl
...
class DSLPIDControl(BaseControl):
    ...
    def __init__(self, drone_model: DroneModel, g: float = 9.8):
        ...
        if (
            ...
            and self.DRONE_MODEL != DroneModel.CF2P
        ):
            print(
                "[ERROR] in DSLPIDControl.__init__(), DSLPIDControl"
                " requires DroneModel.CF2X or DroneModel.CF2P"
            )
            exit()
    ...
    def computeControl(
        ...
        thrust, computed_target_rpy, pos_e = (
    ...
    def _dslPIDPositionControl(
        ...
        cur_rotation = np.array(
            p.getMatrixFromQuaternion(cur_quat)
        ).reshape(3, 3)
        ...
        self.integral_pos_e[2] = np.clip(
            self.integral_pos_e[2], -0.15, 0.15
        )
        #### PID target thrust #####################################
        target_thrust = (
        ...
        scalar_thrust = max(
            0.0, np.dot(target_thrust, cur_rotation[:, 2])
        )
        thrust = (
            math.sqrt(scalar_thrust / (4 * self.KF))
            - self.PWM2RPM_CONST
        ) / self.PWM2RPM_SCALE
        ...
        target_y_ax = np.cross(
            target_z_ax, target_x_c
        ...
        if np.any(np.abs(target_euler) > math.pi):
            print(
                ...
                (
                    "in Control._dslPIDPositionControl(), values"
                    " outside range [-pi,pi]"
                ),
            )
    ...
    def _dslPIDAttitudeControl(
        ...
        w, x, y, z = target_quat
        target_rotation = (
            Rotation.from_quat([w, x, y, z])
        ).as_matrix()
        rot_matrix_e = np.dot(
            (target_rotation.transpose()), cur_rotation
        ...
        self.integral_rpy_e = np.clip(
            self.integral_rpy_e, -1500.0, 1500.0
        )
    ...
    def _one23DInterface(self, thrust):
        ...
        pwm = np.clip(
            (
                np.sqrt(np.array(thrust) / (self.KF * (4 / DIM)))
                - self.PWM2RPM_CONST
            )
            / self.PWM2RPM_SCALE,
            self.MIN_PWM,
