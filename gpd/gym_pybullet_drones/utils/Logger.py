...
class Logger(object):
    ...
    def __init__(
        ...
        self.PREALLOCATED_ARRAYS = (
            False if duration_sec == 0 else True
        )
        ...
        self.states = np.zeros(
            (num_drones, 16, duration_sec * self.LOGGING_FREQ_HZ)
        )  #### 16 states: pos_x,
        # pos_y,
        # pos_z,
        # vel_x,
    ...
    def log(self, drone: int, timestamp, state, control=np.zeros(12)):
        ...
        if current_counter >= self.timestamps.shape[1]:
            self.timestamps = np.concatenate(
                (self.timestamps, np.zeros((self.NUM_DRONES, 1))),
                axis=1,
            )
            self.states = np.concatenate(
                (self.states, np.zeros((self.NUM_DRONES, 16, 1))),
                axis=2,
            )
            self.controls = np.concatenate(
                (self.controls, np.zeros((self.NUM_DRONES, 12, 1))),
                axis=2,
            )
        #### Advance a counter is the matrices have overgrown it ###
        elif (
            not self.PREALLOCATED_ARRAYS
            and self.timestamps.shape[1] > current_counter
        ):
    ...
    def save_as_csv(self, comment: str = ""):
        ...
        t = np.arange(
            0,
            self.timestamps.shape[1] / self.LOGGING_FREQ_HZ,
            1 / self.LOGGING_FREQ_HZ,
        )
        for i in range(self.NUM_DRONES):
            with open(
                csv_dir + "/x" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 0, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/y" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 1, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/z" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 2, :]])
                    ),
                    delimiter=",",
                )
            ####
            with open(
                csv_dir + "/r" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 6, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/p" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 7, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/ya" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 8, :]])
                    ),
                    delimiter=",",
                )
            ####
            with open(
                csv_dir + "/rr" + str(i) + ".csv", "wb"
            ) as out_file:
                rdot = np.hstack(
                    [
                        0,
            ...
            ###
            with open(
                csv_dir + "/vx" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 3, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/vy" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 4, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/vz" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 5, :]])
                    ),
                    delimiter=",",
                )
            ####
            with open(
                csv_dir + "/wx" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 9, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/wy" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 10, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/wz" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 11, :]])
                    ),
                    delimiter=",",
                )
            ####
            with open(
                csv_dir + "/rpm0-" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 12, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/rpm1-" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 13, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/rpm2-" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 14, :]])
                    ),
                    delimiter=",",
                )
            with open(
                csv_dir + "/rpm3-" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
                        np.vstack([t, self.states[i, 15, :]])
                    ),
                    delimiter=",",
                )
            ####
            with open(
                csv_dir + "/pwm0-" + str(i) + ".csv", "wb"
            ) as out_file:
                np.savetxt(
                    out_file,
                    np.transpose(
    ...
    def plot(self, pwm=False):
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 0, :], label="drone_" + str(j)
            )
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 1, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 2, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 6, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 7, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 8, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 9, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 10, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 11, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 3, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 4, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 5, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 12, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        if pwm:
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 13, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        if pwm:
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 14, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        if pwm:
        ...
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(
                t, self.states[j, 15, :], label="drone_" + str(j)
            )
        axs[row, col].set_xlabel("time")
        if pwm:
