...
class TestCustomDroneEnv(unittest.TestCase):
    def setUp(self):
        ...
        self.env = CustomDroneEnv(
            gui=False, num_drones=3
        )  # Assuming no GUI needed for testing
        self.env.reset()

    def test_environment_configuration(self):
        """Test the environment's basic configuration."""
        self.assertEqual(
            self.env.num_drones, 3, "Unexpected number of drones."
        )
        self.assertTrue(
            self.env.physics in ["pyb"], "Unexpected physics engine."
        )
    ...
    def test_action_generation_and_application(self):
        ...
        mock_actions = np.random.uniform(
            -1, 1, (self.env.num_drones, 7)
        )
    ...
    def test_observation_space(self):
        ...
        self.assertEqual(
            ...
            (3, 10),
            "Incorrect observation space shape.",
        )
        self.assertTrue(
            (observation_space.high > 1e3).all()
            or (observation_space.low < -1e3).all(),
            "Observation space bounds are incorrect.",
        )
    ...
    def test_step_outcome(self):
        ...
        action = np.zeros(
            (3, 7)
        )  # Assuming zero action for simplicity
        observations, rewards, dones, infos = self.env.step()
        self.assertEqual(
            observations.shape,
            (3, 10),
            "Incorrect observation shape.",
        )
        self.assertEqual(
            len(rewards), 3, "Incorrect number of rewards."
        )
        self.assertEqual(
            len(dones), 3, "Incorrect number of done flags."
        )
        self.assertIsInstance(
            infos, dict, "Info should be a dictionary."
        )
    ...
    def test_reset_state(self):
        ...
        self.assertEqual(
            observations.shape,
            (3, 10),
            "Reset observation shape is incorrect.",
        )
    ...
    def test_drone_dynamics(self):
        ...
        self.assertFalse(
            np.array_equal(
                initial_observation[:, :3], observations[:, :3]
            ),
            "Drones' positions did not change as expected.",
        )
