...
class BetaAviary(BaseAviary):
    ...
    def __init__(
        ...
        for i in range(self.NUM_DRONES):
            ...
            )  # drone0 will be on 9002, drone1 on 9012, etc.
    ...
    def step(self, action, i):
        ...
        for j in range(self.NUM_DRONES):  # TODO: add multi-drone support
            ...
            fdm_packet = struct.pack(
                ...
                w_body[0],
                -w_body[1],
                -w_body[2],  # double imu_angular_velocity_rpy[3]
                0,
                0,
                0,  # double imu_linear_acceleration_xyz[3]
                1.0,
                0.0,
                0.0,
                0.0,  # double imu_orientation_quat[4] w, x, y, z
                0,
                0,
                0,  # double velocity_xyz[3]
                0,
                0,
                0,  # double position_xyz[3]
                1.0,  # double pressure;
            )
            self.sock[j].sendto(
                fdm_packet, (self.UDP_IP, BASE_PORT_STATE + 10 * (j))
            )  # drone0 will be on 9003, drone1 on 9013, etc.
            ...
            rc_packet = struct.pack(
                ...
                round(pitch),
                round(thro),
                round(yaw),  # roll, pitch, throttle, yaw
                aux1,
                1000,
                1000,
                1000,  # aux 1, ..
            ...
            self.sock[j].sendto(
                rc_packet, (self.UDP_IP, BASE_PORT_RC + 10 * (j))
            )  # drone0 will be on 9004, drone1 on 9014, etc.

            #### PWMs message from Betaflight ##########################
            try:
                data, addr = self.sock_pwm[j].recvfrom(
                    16
                )  # buffer size is 100 bytes (servo_packet size 16)
